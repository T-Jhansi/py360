...
@api_view(['GET'])
@permission_classes([permissions.IsAuthenticated])
def search_case_logs_by_case_number_api(request: Union[Request, HttpRequest]) -> Response:
    """
    Search case logs by case number (case-insensitive)
    Accepts both CASE-001 and case-001 formats
    """
    try:
...
@api_view(['GET'])
@permission_classes([permissions.IsAuthenticated])
def search_case_logs_by_policy_number_api(request: Union[Request, HttpRequest]) -> Response:
    """
    ...
    try:
        ...
        # Get the first renewal case for basic info (since all cases share the same policy)
        first_case = renewal_cases.first()  # type: ignore
